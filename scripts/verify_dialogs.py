...
PROMPT_KEYS = set(["prompt", "response", "cycle"])
...
def verify_line(speaker, key, lines):
    # TODO: separate characters from lines?
...
def verify_prompt(prompt, lines):
    for key, value in prompt.items():
        ...
        if key == "response" and not verify_response(value, lines):
            return False

        if key == "cycle":
            if not type(value) == list:
                print("invalid cycle value: {}", value)
                return False
